...
Az Intune és a Microsoft Enterprise Mobility + Security (EMS) egyedi, integrált [feltételes hozzáférési megoldást](conditional-access.md) kínál az Exchange Serverhez, amely biztosítja, hogy a Mobile-alkalmazások csak akkor férhessenek hozzá az e-mailekhez, ha az eszköz regisztrálva van az Intune-ban. Ezt a típusú e-mail-hozzáférést anélkül implementálhatja, hogy egy másik átjárót helyez üzembe a vállalati hálózat peremén.
...
Ha azonban a felhasználónak egyszerűen hozzá kell férnie a vállalati e-mailekhez és dokumentumokhoz, ami gyakran előfordul a személyes tulajdonú eszközök esetében is, akkor a felhasználónak az Office Mobile apps használatát kell megkövetelni (amelyre alkalmazta az [alkalmazás-védelmi szabályzatokat](app-protection-policies.md) , és kihagyhatja a regisztrálást. az eszköz teljes egészében.  
...
Ebben az esetben a felhasználói élmény nagyon fontos jellemzője az egyszerűség. Emiatt a tabletták általában korlátozott használatú módban vannak megadva az alkalmazottak számára, így az egyetlen üzletági alkalmazás az egyetlen dolog, amelyet az alkalmazott tud használni. Az Intune lehetővé teszi az ilyen korlátozott használatú módban történő futtatásra konfigurálható megosztott [iOS-és Android-](device-profiles.md) eszközök tömeges kiépítését, védelmét és központi felügyeletét.
