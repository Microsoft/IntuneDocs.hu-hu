...
> [!IMPORTANT]
> Mobilalkalmazás-kezelési házirendeket hozhat létre az Office 365-szolgáltatásokhoz kapcsolódó Office-mobilalkalmazások számára. A helyszíni Exchange-postaládákhoz való hozzáférést úgy is biztosíthatja, ha az iOS/iPadOS és az Android rendszerhez készült Intune app Protection-szabályzatokat hoz létre a modern hibrid hitelesítéssel. A szolgáltatás használata előtt győződjön meg arról, hogy megfelel az [iOS/iPadOS és az Android rendszerhez készült Outlook követelményeinek](https://technet.microsoft.com/library/mt846639(v=exchg.160).aspx). Az alkalmazásvédelmi szabályzatok a helyszíni Exchange-hez vagy a SharePoint-szolgáltatásokhoz kapcsolódó egyéb alkalmazások esetében nem támogatottak.
...
Az Intune app Protection-házirendek platform támogatja az Office Mobile Application Platform-támogatást az Android és iOS/iPadOS eszközökhöz. További információt az **Office rendszerkövetelményeit** ismertető témakör [mobilalkalmazásokkal](https://products.office.com/office-system-requirements#coreui-contentrichblock-9r05pwg) foglalkozó szakaszában talál.
...
Ha engedélyezve van, az iOS/iPadOS és az Android rendszerhez készült OneDrive-és SharePoint-alkalmazások alapértelmezés szerint a kiválasztott beállításokkal vannak védve. Az informatikai szakemberek az Intune-konzolon szerkeszthetik ezt a szabályzatot, ahol további célalkalmazásokat adhatnak hozzá, és bármely beállítását módosíthatják. 
...
- **A PIN-kód azonos közzétevő alkalmazásai között van megosztva a használhatóság javítása érdekében:**<br> IOS/iPadOS egyetlen alkalmazás-PIN-kód van megosztva **az azonos alkalmazás-közzétevőhöz**tartozó alkalmazások között. Az összes Microsoft-alkalmazás például ugyanazt a PIN-kódot használja. Androidon minden alkalmazás egyetlen közös alkalmazásszintű PIN-kódot használ.
- **Az eszköz újraindítása utáni *(perc) viselkedés utáni újraellenőrzési követelmények* :**<br> Az időzítő nyomon követi, hogy a rendszer hány perc inaktivitás után jelenítse meg az Intune-alkalmazás PIN-kódját, vagy a vállalati hitelesítő adatok megadását. IOS/iPadOS esetén az eszköz újraindítása nem érinti az időzítőt. Így az eszköz újraindítása nem befolyásolja azon percek számát, ameddig a felhasználó inaktív volt egy, az Intune PIN-kódjával (vagy vállalati hitelesítő adatokkal) megcélozott iOS/iPadOS alkalmazásból. Androidon az időzítő visszaáll az eszköz újraindítására. Ilyenkor az Intune PIN-kódját (vagy vállalati hitelesítő adatokkal) tartalmazó Android-alkalmazások valószínűleg rákérdeznek az alkalmazás PIN-kódjára vagy a vállalati hitelesítő adatokra, függetlenül attól, hogy az **eszköz újraindítása után**megtörtént-e a hozzáférési követelmények ismételt ellenőrzésének (perc) beállítása.  
...
IOS/iPadOS esetén az alkalmazás szintű PIN-kód adatait a kulcstartó tárolja, amely azonos közzétevővel, például az első fél Microsoft-alkalmazásaival közösen van megosztva. A PIN-kód adatai egy végfelhasználói fiókhoz is kötődnek. Egy alkalmazás szelektív törlése nem érintheti a másik alkalmazást. 
...
A MAM (iOS/iPadOS) jelenleg lehetővé teszi, hogy az alkalmazás szintű PIN-kód alfanumerikus és speciális karaktereket használjon (úgynevezett PIN-kód), amely az [iOS-hez készült INTUNE SDK](../developer/app-sdk-ios.md)integrálásához az alkalmazások (például WXP, Outlook, Managed Browser, Yammer) részvételét igényli. Az integráció hiányában a PIN-jelszóbeállítások nem lesznek megfelelően megkövetelve a megcélzott alkalmazásoknál. Ez a szolgáltatás az iOS-hez készült Intune SDK következő verziójában jelent meg: 7.1.12.

A funkció támogatásához és az iOS/iPadOS Intune SDK korábbi verzióival való visszamenőleges kompatibilitás érdekében az 7.1.12 +-ban lévő összes PIN-kód (numerikus vagy PIN-kód) külön van kezelve az SDK korábbi verzióiban található numerikus PIN-kóddal. Így ha egy eszközön olyan alkalmazások találhatók, melyek az iOS-hez készült Intune SDK 7.1.12-esnél korábbi ÉS 7.1.12-esnél újabb verziójával rendelkeznek, és ugyanattól a gyártótól származnak, ezekhez két PIN-kódot kell beállítani. A két PIN-kód (az egyes alkalmazásokhoz) nem kapcsolódik semmilyen módon (azaz be kell tartania az alkalmazásra alkalmazott adatvédelmi szabályzatot). Így *csak* abban az esetben, ha az a és a B alkalmazáshoz ugyanazok a szabályzatok érvényesek (a PIN-kód tekintetében), a felhasználó kétszer is beállíthatja ugyanazt a PIN-kódot. 

Ez a viselkedés kizárólag az Intune Mobile App managementtel engedélyezett iOS/iPadOS-alkalmazások PIN-kódjára vonatkozik. Az idő múlásával, mivel az alkalmazások az iOS/iPadOS Intune SDK újabb verzióit fogadják el, az azonos közzétevőtől származó alkalmazásokban kétszer kell beállítani a PIN-kódot. Erre az alábbi megjegyzésben találhat egy példát.

  >[!NOTE]
  > Ha például az A alkalmazás egy, a 7.1.12 előtti verziót használ, és a B alkalmazás az azonos közzétevőtől származó 7.1.12 nagyobb vagy azzal egyenlő, a végfelhasználónak külön kell beállítania a PIN-kódokat az A és B esetében, ha mindkettő egy iOS-vagy iPadOS-eszközre van telepítve.
...
**Hogyan működik az Intune-adattitkosítási folyamat**<br> Tekintse meg az [Android-alkalmazások védelmi](app-protection-policy-settings-android.md) házirendjének beállításait és az [iOS/iPadOS alkalmazás-védelmi szabályzat beállításait](app-protection-policy-settings-ios.md) a titkosítási alkalmazás védelmi házirendjének beállításával kapcsolatos részletes információkért.
...
Ennek a folyamatnak a célja, hogy az alkalmazás szintjén biztonságosan és védelemmel lássa el a szervezet adatait az alkalmazáson belül. Ez a funkció csak iOS/iPadOS esetén érhető el, és az iOS/iPadOS, 9.0.1 vagy újabb verziójú Intune SDK-t integráló alkalmazások részvételét igényli. Az SDK-integráció szükséges a viselkedés kényszeríthetőségéhez a megcélzott alkalmazásoknál. Ez az integráció fokozatosan történik, és az egyes alkalmazáscsapatoktól függ. Néhány alkalmazás, amely ezek között szerepelhet: WXP, Outlook, Managed Browser és Yammer.
  
### <a name="ios-share-extension"></a>iOS-megosztási bővítmény
Az iOS/iPadOS megosztási bővítmény használatával a nem felügyelt alkalmazásokban is megnyithat munkahelyi vagy iskolai adatokkal, még a **felügyelt alkalmazásokhoz** beállított adatátviteli szabályzattal, vagy akár egyetlen **alkalmazással**sem. Az Intune app Protection-szabályzat nem tudja szabályozni az iOS/iPadOS-megosztás kiterjesztését az eszköz kezelése nélkül. Ezért az _**Intune titkosítja a „céges” adatokat, mielőtt az alkalmazáson kívül megosztaná**_ . Ezt a titkosítási viselkedést a felügyelt alkalmazáson kívüli "céges" fájl megnyitására tett kísérlettel ellenőrizheti. A fájlnak titkosítottnak kell lennie, így a felügyelt alkalmazáson kívül mással nem nyitható meg.
