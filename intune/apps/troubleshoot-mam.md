...
| -- | -- | -- |
| A Skype Vállalati verzióra nem vonatkozó szabályzat | A Azure Portalban készült, eszközök regisztrációja nélküli alkalmazás-védelmi szabályzat nem vonatkozik a Skype vállalati alkalmazásra iOS/iPadOS és Android rendszerű eszközökön. | A Skype Vállalati verziót a modern hitelesítésre kell beállítani.  Kövesse a [Bérlő engedélyezése modern hitelesítéshez](https://social.technet.microsoft.com/wiki/contents/articles/34339.skype-for-business-online-enable-your-tenant-for-modern-authentication.aspx) című részben található utasításokat a modern hitelesítés beállításához Skype-hoz. |
...
| Az alkalmazásvédelmi szabályzat nem működik a DEP-pel | Az alkalmazásvédelmi szabályzat nem lép érvénybe az Apple DEP-eszközökön. | Győződjön meg róla, hogy használja-e a Felhasználói affinitás funkciót az Apple Eszközregisztrációs programmal (DEP). Felhasználói affinitásra minden olyan alkalmazás esetében szükség van, amely felhasználói hitelesítést igényel DEP alatt. <br><br>Az iOS/iPadOS-alapú DEP-regisztrációval kapcsolatos további információkért tekintse meg az [iOS/iPadOS-eszközök automatikus regisztrálását az Apple Készülékregisztrációs programával](../enrollment/device-enrollment-program-enroll-ios.md) .|
| Az adatátviteli szabályzat nem működik az iOS/iPadOS | Az **alkalmazás átviheti** az adatátvitelt más alkalmazásokba, és **lehetővé teszi az alkalmazások számára más alkalmazásokból érkező adatok fogadását** , nem sikerült az adatátvitelt az iOS/iPadOS szolgáltatásban kezelni. | Lásd: az [iOS/iPadOS alkalmazások közötti adatátvitel kezelése Microsoft Intuneban](data-transfer-between-apps-manage-ios.md). |
