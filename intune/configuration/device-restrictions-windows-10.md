...
- **Trusted app installation**: Choose if non-Microsoft Store apps can be installed, also known as sideloading. Sideloading is installing, and then running or testing an app that isn't certified by the Microsoft Store. For example, an app that is internal to your company only. A választható lehetőségek:
...
- **Developer unlock**: Allow Windows developer settings, such as allowing sideloaded apps to be modified by end users. A választható lehetőségek:
  - **Not configured** (default): Uses the OS default.
...
- **Apps from store only**: This setting determines the user experience when users install apps from places other than the Microsoft Store. A választható lehetőségek:
...
- **Cellular data channel**: Choose if end users can use data, like browsing the web, when connected to a cellular network. A választható lehetőségek:
...
- **Printer discovery URL**: Enter the URL for finding cloud printers. Például írja be a következőt: `https://cloudprinterdiscovery.contoso.com`.
- **Printer access authority URL**: Enter the authentication endpoint URL to get OAuth tokens. Például írja be a következőt: `https://azuretenant.contoso.com/adfs`.
- **Azure native client app GUID**: Enter the GUID of a client application allowed to get OAuth tokens from the OAuthAuthority. Például írja be a következőt: `E1CF1107-FF90-4228-93BF-26052DD2C714`.
- **Print service resource URI**: Enter the OAuth resource URI for print service configured in the Azure portal. Például írja be a következőt: `http://MicrosoftEnterpriseCloudPrint/CloudPrint`.
- **Maximum printers to query**: Enter the maximum number of printers that you want to be queried. The default value is `20`.
- **Printer discovery service resource URI**: Enter the OAuth resource URI for printer discovery service configured in the Azure portal. Például írja be a következőt: `http://MopriaDiscoveryService/CloudPrint`.
...
- **Turn on GDI scaling for apps**: **Add** the legacy apps that you want GDI DPI scaling turned on. Például írja be a következőt: `filename.exe` vagy `%ProgramFiles%\Path\Filename.exe`.

  GDI DPI scaling is turned on for all legacy applications in your list.

- **Turn off GDI scaling for apps**: **Add** the legacy apps that you want GDI DPI scaling turned off. Például írja be a következőt: `filename.exe` vagy `%ProgramFiles%\Path\Filename.exe`.
...
- **Ink Workspace**: Choose if and how user access the ink workspace. A választható lehetőségek:
...
- **Locked screen picture URL (desktop only)** : Enter the URL to a picture in JPG, JPEG, or PNG format that's used as the Windows lock screen wallpaper. Például írja be a következőt: `https://contoso.com/image.png`. This setting locks the image, and can't be changed afterwards.
...
The available settings change depending on what you choose. A választható lehetőségek:
...
- **Start Microsoft Edge with**: Choose which pages open when Microsoft Edge starts. A választható lehetőségek:
...
- **New Tab URL**: Enter the URL to open on the New Tab page. Például írja be a következőt: `https://www.bing.com` vagy `https://www.contoso.com`.

- **Home button**: Choose what happens when the home button is selected. A választható lehetőségek:
  ...
  - **Home button URL**: Enter the URL to open. Például írja be a következőt: `https://www.bing.com` vagy `https://www.contoso.com`.
...
- **First Run Experience URL list location** (Windows 10 Mobile only): Enter the URL that points to the XML file containing the first run page URL(s). Például írja be a következőt: `https://www.contoso.com/sites.xml`.
...
- **Enterprise mode site list location** (Desktop only): Enter the URL that points to the XML file containing a list of web sites that open in Enterprise mode. Users can't change this list. Például írja be a következőt: `https://www.contoso.com/sites.xml`.
- **Message when opening sites in Internet Explorer**: Use this setting to configure Microsoft Edge to show a notification before a site opens in Internet Explorer 11. A választható lehetőségek:
...
- **Show Favorites bar**: Choose what happens to the favorites bar on any Microsoft Edge page. A választható lehetőségek:
...
- **Default search engine**: Choose the default search engine on the device. A végfelhasználók ezt az értéket bármikor módosíthatják. A választható lehetőségek:
  - Search engine in client Microsoft Edge settings
  - Bing
  - Google
  - Yahoo
  - Custom value: In **OpenSearch Xml URL**, enter an HTTPS URL with the XML file that includes the short name and the URL to the search engine, at minimum. Például írja be a következőt: `https://www.contoso.com/opensearch.xml`.
...
- **Sync browser settings between user's devices**: Choose how you want to sync browser settings between devices. A választható lehetőségek:
...
- **Cookies**: Choose how cookies are handled in the web browser. A választható lehetőségek:
...
- **Use manual proxy server**: Choose **Allow** to manually enter the  name or IP address, and TCP port number of a proxy server. **Not configured** (default) doesn't let you manually enter details of a proxy server.
  ...
  - **Proxy exceptions**: Enter any URLs that must not use the proxy server. Az egyes címeket pontosvesszővel választhatja el egymástól.
...
- **Password**: **Require** the end user to enter a password to access the device. **Not configured** (default) allows access to the device without a password. Applies to local accounts only. Domain account passwords remain configured by Active Directory (AD) and Azure AD.
  ...
  - **Required password type**: Choose the type of password. A választható lehetőségek:
  ...
  - **Simple passwords**: Set to **Block** so users can't create simple passwords, such as `1234` or `1111`. Set to **Not configured** (default) to let users create passwords like `1234` or `1111`. Ez a beállítás a Windows-képjelszavak használatát is engedélyezi vagy letiltja.
...
- **Web sign-in**: Enables Windows sign in support for non-ADFS (Active Directory Federation Services) federated providers, such as Security Assertion Markup Language (SAML). SAML uses secure tokens that provide web browsers a single sign-on (SSO) experience. A választható lehetőségek:
...
- **Radios**: Some apps use radios (for example, Bluetooth) in your device to send and receive data and need to turn these radios on or off. Megadhatja, hogy az alkalmazás kezelheti-e ezeket az antennákat.
...
- **Desktop background picture URL (Desktop only)** : Enter the URL to a picture in .jpg, .jpeg or .png format that you want to use as the Windows desktop wallpaper. A felhasználók nem módosíthatják a képet. Például írja be a következőt: `https://contoso.com/logo.png`.
...
- **Share usage data**: Choose the level of diagnostic data that's submitted. A választható lehetőségek:
...
- **Send Microsoft Edge browsing data to Microsoft 365 Analytics**: To use this feature, set the **Share usage data** settings to **Enhanced** or **Full**. This feature controls what data Microsoft Edge sends to Microsoft 365 Analytics for enterprise devices with a configured commercial ID. A választható lehetőségek:
...
- **Telemetry proxy server**: Enter the fully qualified domain name (FQDN) or IP address of a proxy server to forward Connected User Experiences and Telemetry requests, using a Secure Sockets Layer (SSL) connection. a következő formátumban: *kiszolgáló*:*port*. If the named proxy fails, or if a proxy isn't entered when enabling this policy, the Connected User Experiences and Telemetry data isn't sent, and stays on the local device.
...
- **Safe Search (mobile only)** : Control how Cortana filters adult content in search results. A választható lehetőségek:
...
- **Most used apps**: **Block** hides the most used apps from showing on the start menu. A Gépház alkalmazásban is letiltja a megfelelő váltógombot. **Not configured** (default) shows the most used apps.
- **Recently added apps**: **Block** hides recently added apps from showing on the start menu. A Gépház alkalmazásban is letiltja a megfelelő váltógombot. **Not configured** (default) shows the recently added apps on the start menu.
- **Start screen mode**: Choose how the start screen is shown. A választható lehetőségek:
...
- **Recently opened items in Jump Lists**: **Block** hides recent jump lists from being shown on the start menu and taskbar. A Gépház alkalmazásban is letiltja a megfelelő váltógombot. **Not configured** (default) shows recently opened items in the jumplists.
- **App list**: Choose how the all apps lists are shown. A választható lehetőségek:
...
- **Documents on Start**: Hide or show the Documents folder in the Windows Start menu. A választható lehetőségek:
...
- **Downloads on Start**: Hide or show the Downloads folder in the Windows Start menu. A választható lehetőségek:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **File Explorer on Start**: Hide or show File Explorer in the Windows Start menu. A választható lehetőségek:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **HomeGroup on Start**: Hide or show the HomeGroup shortcut in the Windows Start menu. A választható lehetőségek:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **Music on Start**: Hide or show the Music folder in the Windows Start menu. A választható lehetőségek:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **Network on Start**: Hide or show Network in the Windows Start menu. A választható lehetőségek:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **Personal folder on Start**: Hide or show Personal folder in the Windows Start menu. A választható lehetőségek:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **Pictures on Start**: Hide or show the folder for pictures in the Windows Start menu. A választható lehetőségek:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **Settings on Start**: Hide or show the Settings shortcut in the Windows Start menu. A választható lehetőségek:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
- **Videos on Start**: Hide or show the folder for videos in the Windows Start menu. A választható lehetőségek:
  - **Not configured** (default): No setting is forced. Users choose to show or hide the shortcut.
  - **Hide**: The shortcut is hidden, and disables the setting in the Settings app.
  - **Show**: The shortcut is shown, and disables the setting in the Settings app.
...
- **Network Inspection System (NIS)** : NIS helps to protect devices against network-based exploits. A Microsoft Endpoint Protection-központból származó ismert sebezhető pontok mintázatai alapján segít észlelni és blokkolni a rosszindulatú hálózati forgalmat.
...
- **Security intelligence update interval (in hours)** : Enter the interval that Defender checks for new security intelligence, from 0-24. A választható lehetőségek:
...
- **Monitor file and program activity**: Allows Defender to monitor file and program activity on devices. A választható lehetőségek:
...
- **Prompt users before sample submission**: Controls whether potentially malicious files that might require further analysis are automatically sent to Microsoft. A választható lehetőségek:
...
- **Type of system scan to perform**: Schedule a system scan, including the level of scanning, and the day and time to run the scan. A választható lehetőségek:
...
- **Detect potentially unwanted applications**: Choose the level of protection when Windows detects potentially unwanted applications. A választható lehetőségek:
...
- **Files and folders to exclude from scans and real-time protection**: Adds one or more files and folders like **C:\Path** or **%ProgramFiles%\Path\filename.exe** to the exclusions list. A rendszer a valós idejű és ütemezett vizsgálatok során nem vizsgálja ezeket a fájlokat és mappákat.
