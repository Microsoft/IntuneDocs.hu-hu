...
Az eszköz konfigurációs profiljainak részeként [beállíthatja a kézbesítés optimalizálását](delivery-optimization-settings.md) .
...
> [!NOTE]
> A Microsoft Intune segítségével kezelheti a Windows rendszerű számítógépeket mobileszköz- [felügyeleti (Mdm)](windows-enroll.md) vagy Intune-ügyfélszoftverrel rendelkező számítógépekként. A Microsoft azt javasolja, hogy az ügyfelek, amikor csak lehetséges, [a Mdm-kezelési megoldást használják](windows-enroll.md) . Ha ezt a módszert felügyeli, a BranchCache nem támogatott. További információ: a [Windows rendszerű számítógépek számítógépként vagy mobileszközökként való kezelésének összehasonlítása](pc-management-comparison.md).
