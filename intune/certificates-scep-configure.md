...
- **Azure-ad Application proxy** (nem kötelező) – a dedikált webalkalmazás-proxy (WAP) kiszolgáló helyett az Azure AD Application Proxy használhatja a NDES URL-címének interneten való közzétételéhez. Ez lehetővé teszi, hogy az intranetes és az internetes eszközök is megkapják a tanúsítványokat. További információért lásd: [Helyszíni alkalmazások biztonságos távoli elérése](https://docs.microsoft.com/azure/active-directory/manage-apps/application-proxy). 

- **Webalkalmazás-proxy kiszolgáló** (nem kötelező) – Windows Server 2012 R2 vagy újabb rendszert futtató kiszolgáló használata webalkalmazás-proxy (WAP) kiszolgálóként a NDES URL-címének közzétételéhez az interneten.  Ez lehetővé teszi, hogy az intranetes és az internetes eszközök is megkapják a tanúsítványokat.
...
3. Válassza a **Tanúsítványsablonok** csomópontot, majd kattintson a **művelet** > **kezelése**lehetőségre.
...
-  > A bővítmények**alkalmazás** -házirendjeinek tartalmaznia kell az **ügyfél-hitelesítést**
-  > **A kérelemben adja meg a**tulajdonos nevét.
...
-  > A bővítmények**alkalmazás** -házirendjeinek **kiszolgálói hitelesítést** kell tartalmazniuk
-  > **A kérelemben adja meg a**tulajdonos nevét.
...
2. Tegye közzé a Hitelesítésszolgáltató beépülő modullal a tanúsítványsablont a vállalati hitelesítésszolgáltatón. Válassza a **Tanúsítványsablonok** csomópontot, válassza a **művelet** > **új** > **kiállítandó**tanúsítványsablon lehetőséget, majd válassza ki az előző szakaszban létrehozott tanúsítványsablont.  
...
3. Konfigurálja az IIS-kérelmek szűrését az NDES szolgáltatás által fogadott hosszú URL-címek (lekérdezések) támogatásához az IIS-ben.
   1. Az IIS-kezelőben válassza ki az **alapértelmezett** > webhely-**kérelem szűrésének** > **szerkesztése funkció beállítást** a **kérelmek szűrési beállításainak szerkesztése** lap megnyitásához.  
...
1. Jelentkezzen be az [Intune](https://aka.ms/intuneportal) -portálra egy olyan fiókkal, amely rendelkezik Intune-jogokkal.  
...
A szolgáltatás futásának ellenőrzéséhez nyisson meg egy böngészőt, és írja be az alábbi URL-t. **403** hibát kell visszaadnia:`http://<FQDN_of_your_NDES_server>/certsrv/mscep/mscep.dll`  
